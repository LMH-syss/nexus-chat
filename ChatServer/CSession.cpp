    ...
    try {
      if (ec) {
        if (ec == boost::asio::error::eof ||
          ...
          std::cout << "session disconnected, session_id is " << _session_id
                    << std::endl;
        } else {
          std::cout << "handle read body failed, error is " << ec.message()
                    << std::endl;
        }
        Close();
      ...
      if (bytes_transfered < total_len) {
        std::cout << "read body length not match, read [" << bytes_transfered
...
void CSession::asyncReadLen(
  ...
  _socket.async_read_some(
      ...
      [self, read_len, total_len, handler](const boost::system::error_code &ec,
...
void CSession::asyncWriteQueued() {
  ...
  boost::asio::async_write(
      _socket, boost::asio::buffer(send_node->_data, send_node->_total_len),
...
void CSession::UpdateHeartbeat() { _last_heartbeat = std::time(nullptr); }
