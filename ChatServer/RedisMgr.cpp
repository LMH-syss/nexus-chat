...
bool RedisMgr::SetEx(const std::string& key, const std::string& value, int seconds) {
    ...
    auto reply = (redisReply*)redisCommand(connect, "SETEX %s %d %s", key.c_str(), seconds, value.c_str());
    if (reply == NULL) {
        std::cout << "Execut command [ SETEX " << key << "  " << value << " ] failure ! " << std::endl;
        _con_pool->returnConnection(connect);
        return false;
    }
...
bool RedisMgr::HSet(const char* key, const char* hkey, const char* hvalue, size_t hvaluelen) {
    ...
    argvlen[0] = 4;
    argv[1] = key;
    argvlen[1] = std::strlen(key);
    argv[2] = hkey;
    argvlen[2] = std::strlen(hkey);
    argv[3] = hvalue;
    argvlen[3] = hvaluelen;
